...
## En cours
M0-T22 `evals-core` en cours : cinq revues sécurité BLOCK successives sur la même classe (écart entre texte relu et donnée évaluée). Changements d'approche consignés (règle « même échec deux fois ») : V4 (arbre YAML validé avant conversion), puis V10 (liste d'admission des caractères au lieu de listes de refus). Option écartée pour l'instant, réversible : ASCII strict avec échappements pour tout le reste (plus sûr, mais le français devient illisible dans les cas et l'exemple du skill `agent-evals` devrait être réécrit).
...
2. Appliquer les propositions 0001 (harnais), 0002 (CLAUDE.md), 0003 (garde make et bac à sable), 0004 (`post_edit_check` hors du dépôt) puis 0005 (suites d'eval gelées en phase impl, `CODEOWNERS`, **condition avant M0-T23**), redémarrer Claude Code ; activer « Require review from Code Owners » sur la branche par défaut. M0-T01 a été faite sans elles (écarts consignés ci-dessus).
