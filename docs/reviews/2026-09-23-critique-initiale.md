...
- **Squelette RunLoop** (`.claude/skills/loop-engineering/references/temporal-loop-skeleton.md`) : `AwaitApproval` s'arrête au premier signal, même avec un mauvais hash, et ne vérifie ni identité, ni quorum, ni exclusion de l'auteur ; le compteur de stagnation n'est pas remis à zéro au changement de stratégie (la troisième stratégie n'est jamais atteinte). À corriger avant d'écrire les tests de M0.
