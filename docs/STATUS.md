...
- Proposition de harnais (à rédiger en `docs/proposals/0004`) : `.claude/hooks/post_edit_check.py` (lignes 31 et 37) lance `go vet` depuis le dépôt pour un fichier édité hors du dépôt (copies de vérification dans le scratchpad) et bloque avec « directory not found » ; correctif : sortir en 0 si le chemin est hors de `$CLAUDE_PROJECT_DIR`.
